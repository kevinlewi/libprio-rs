...
impl<I: Idpf<2, 2>> Aggregator for Hits<I> {
    ...
    fn prepare_preprocess<M: IntoIterator<Item = Self::PrepareMessage>>(
        ...
        Ok(HitsPrepareMessage(output.unwrap()))
    }
    ...
    fn prepare_step(
        ...
        input: Option<HitsPrepareMessage<I::Field>>,
    ) -> PrepareTransition<
...
mod tests {
    ...
    fn eval_vdaf<I: Idpf<2, 2>>(
        ...
        for state in state1.into_iter() {
            let (state, msg) =
                hits.prepare_next(state, hits.prepare_preprocess(round1.clone())?)?;
            state2.push(state);
            round2.push(msg);
        }
        ...
        for state in state2.into_iter() {
            let output_share =
                hits.prepare_finish(state, hits.prepare_preprocess(round2.clone())?)?;
